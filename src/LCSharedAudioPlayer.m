...
static NSMutableSet *processedViewControllers = nil;
...
void swizzled_viewDidAppear(id self, SEL _cmd, BOOL animated) {
    ...
    // Skip UIAlertController and other system view controllers
    ...
    // Track by view controller instance pointer
    NSString *vcPointer = [NSString stringWithFormat:@"%p", vc];
    if ([processedViewControllers containsObject:vcPointer]) {
        return;
    }
    
    [processedViewControllers addObject:vcPointer];
...
static void initialize() {
    ...
    processedViewControllers = [NSMutableSet new];
